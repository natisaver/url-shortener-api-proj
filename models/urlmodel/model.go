...
type URL struct {
	...
	// 3. smaller space usage compared to storing a composite, using a primitive type like long/BIGINT (8 bytes) will have a dedicated chunk allocated, compared to composite types that will dedicate a much larger chunk (do note this is different from varchar which has 255 bytes allocated)
}
